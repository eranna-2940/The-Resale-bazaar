...
export default function Productdetails() {
  ...
  const [datta, setDatta] = useState(null); // Initialize datta state
  const [firstImage, setFirstImage] = useState(null); // Initialize firstImage state
  ...
  useEffect(() => {
    if (!productdetails) {
      axios
        .get(
          `${process.env.REACT_APP_HOST}${process.env.REACT_APP_PORT}/product/${id}`
        )
        .then((response) => {
  ...
  useEffect(() => {
    if (productdetails && productdetails.image) {
      try {
        const parsedImages = JSON.parse(productdetails.image);
        setDatta(parsedImages);
        setFirstImage(parsedImages[0]); // Set the first image from the parsed images
      } catch (error) {
