...
export default function Orders() {
  ...
  const sellerProductCounts = sellers.map(seller => {
    const products = allProducts.filter(product => product.seller_id === seller.user_id);
    return {
      ...seller,
      productCount: products.length,
      soldCount: products.filter(p => orders.some(o => o.product_id === p.id && o.order_status === 'delivered')).length,
      unsoldCount: products.filter(p => !orders.some(o => o.product_id === p.id && o.order_status === 'delivered')).length,
    };
  });
        ...
        <div className="d-lg-flex justify-content-around p-2 ps-lg-5 pe-lg-5">
          ...
          <div className="col-xs-12 col-md-12 col-lg-9 p-lg-4 p-2">
            {sessionStorage.getItem('token') === "admin" ? (
              <div>
                <h3 className="mb-4">All Sellers and Product Information</h3>
                <table className="table table-hover">
                  <thead>
                    <tr>
                      <th>Seller Name</th>
                  ...
                  <tbody>
                    {sellerProductCounts.map((seller, index) => (
                      <tr key={index}>
                        <td>{`${seller.firstname} ${seller.lastname}`}</td>
                        <td>{seller.productCount}</td>
                        <td>{seller.soldCount}</td>
                        <td>{seller.unsoldCount}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
