...
export default function Addnewproduct() {
  ...
  const placeholders = ['Cover', 'Back', 'Detail', 'Forward', 'Label', 'Side', 'Flaw'];
...
const allDivs = placeholders.map((placeholder, index) => (
  <div className="col-6 col-md-2 mb-3" key={index}>
    <div className="card position-relative" style={{ height: '150px' }}>
      {media.images[index] ? (
        <>
          <img src={URL.createObjectURL(media.images[index])} alt={`upload-${index}`} className="card-img-top" style={{ height: '100%' }} />
          <button
            type="button"
            className="btn-close rounded-circle bg-white position-absolute top-0 end-0 m-2"
          ...
          ></button>
          <div className="placeholder-caption text-center" style={{ position: 'absolute', bottom: '0', width: '100%', background: 'rgba(255, 255, 255, 0.7)', padding: '5px 0', fontSize: '14px', color: '#888' }}>
            {placeholder}
          </div>
        </>
      ) : (
...
const nextAvailableIndex = media.images.findIndex(image => image === null);
  if (nextAvailableIndex !== -1 && nextAvailableIndex < totalPlaceholders) {
    allDivs.splice(nextAvailableIndex, 0, (
      <div className="col-6 col-md-2 mb-3" key="addPhoto">
        <div className="card d-flex justify-content-center align-items-center" style={{ height: '150px' }}>
          <label className="w-100 text-center" htmlFor="addPhotoInput">
            <svg width="28" height="28" className="mb-2">
  ...
  allDivs.push(
    <div className="col-6 col-md-2 mb-3" key="addVideo">
      <div className="card d-flex justify-content-center align-items-center" style={{ height: '150px' }}>
        {media.video ? (
          <>
            <video controls src={URL.createObjectURL(media.video)} style={{ height: '100%' }} />
            <button
              type="button"
              className="btn-close rounded-circle bg-white position-absolute top-0 end-0 m-2"
              style={{ padding: '5px', fontSize: '16px' }}
  ...
  return (
    <div className="fullscreen">
      ...
      <p>Add up to {totalPlaceholders} photos and 1 video in JPEG, PNG, MP4, or WEBM format</p>
